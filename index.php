...
<body>
  <section class="sort">
    ...
    <ul>
      <li>
        <a href="/?sort=publish_date  ASC<?= ($params != '') ? '&' . $params : '' ?>"><img width="20px"
            src="images\icons\asc-sort.png" alt="asc"></a>
        <a href="/?sort= publish_date DESC <?= ($params != '') ? '&' . $params : '' ?>"><img width="20px"
            src="images\icons\desc-sort.png" alt="desc"></a>
      ...
      <ul class="pagination">
        ...
        for ($i = 1; $i <= ceil($count_news / $paginate_count); $i++) { ?>
          <li class="page-item"><a class="page-link"
              href="?page=<?= $i ?><?= $id_cat ? '&cat=' . $id_cat : '' ?> <?= $sort ? '&sort=' . $sort : '' ?> ">
              <?= $i; ?>
            </a></li>
        <?php } ?>
